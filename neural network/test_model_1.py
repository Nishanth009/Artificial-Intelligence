...
## create dataset
dataset =   np.asarray([[4.7810836,2.550537003,0],
			[4.465489372,2.362125076,0],
			[4.396561688,2.400293529,0],
			[5.38807019,2.850220317,0],
			[5.06407232,2.005305973,0],
			[4.627531214,5.759262235,1],
			[2.332441248,4.088626775,1],
			[2.922596716,0.77106367,1],
			[8.675418651,-0.242068655,1],
			[8.673756466,3.508563011,1]])
...
model = FCN(inputs=2, outputs=1, hidden_layers=1, hidden_nodes=[32], 
...
model.fit(X, y.reshape(-1,1),learning_rate=0.2, regularizer=0.01, epochs=10000)
